...
function showGroupFaction(side) {
     return new Promise ((resolve, reject) => {
         setTimeout(()=>{
             resolve(items.filter(product => product.side === side))
         }, 2000)
     })
}

function showSearchedProduct(id) {
    return new Promise ((resolve, reject) => {
        setTimeout(()=>{
            resolve(items.find(product => product.id === id))
        }, 2000)
    })
}

export {showProduct, showSearchedProduct, showGroupFaction}